...
public class EnemyArcher : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        char_level = PlayerPrefs.GetInt("level");
        ...
        //Level = 1;
        Exp = 0;
        HP = 150 + ((Level - 1) * 50);      //Scaling for different levels
        SP = 50;
        Atk = 60 + ((Level - 1) * 20);
        Def = 15 + ((Level - 1) * 5);
        Spd = 15;// + ((Level - 1) * 3);

        PlayerPrefs.SetInt("enemyArrow", Atk);
    ...
    void Update()
    {
        ...
        y_check = Mathf.Abs(transform.position.y - target.transform.position.y);
        if (distance <= attack_range && !attacking && y_check < 0.1 && y_check > -0.1)
        {
            attacking = true;
        ...
        //chase function
        else if (chase && !defeat && !attacking && !isImmobile)
        {
            ...
            Vector3 targetDir = target.position - transform.position;
            if (targetDir.y > 0.01f && distance > 4f)
                enemy.transform.Translate(0, speed * Time.deltaTime, 0);
            else if (targetDir.y < -0.01f && distance > 4f)
                enemy.transform.Translate(0, -speed * Time.deltaTime, 0);
            if (targetDir.x < -0.01f && distance > 4f)
            {
                enemy.transform.Translate(-speed * Time.deltaTime, 0, 0);
                if (facingLeft == false)
                    Flip();
            }
            else if (targetDir.x > 0.01f && distance > 4f)
            {
                enemy.transform.Translate(speed * Time.deltaTime, 0, 0);
                if (facingLeft)
                    Flip();
        ...
        if (attacking)
        {
            ...
            else
            {
                ...
                if (!facingLeft)
                {
                    ...
                    EnemySoundManager.PlaySound("enemyArcherAttack");
                }
                else
                {
        ...
        if (HP <= 0 && !defeat)
        {
            ...
            player.SendMessageUpwards("enemyDefeated", 50 + ((Level - 1) * 10));            //Scaling given gold and exp for different levels
            player.SendMessageUpwards("addGold", 100 + ((Level - 1) * 100));
