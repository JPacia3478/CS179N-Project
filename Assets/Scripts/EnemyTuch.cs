...
public class EnemyTuch : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        hit = false;
        if (invulCount >= 60 && invuln)             //Invulnerability period
        {
            invuln = false;
            invulCount = 0;
        }
		...
		if(HP <= 0 && !defeat)                      //When enemy is defeated
		{
            HP = 0;
			defeat = true;
            e_animator.SetTrigger("EnemyDeath");
            EnemySoundManager.PlaySound("enemySwordDeath");
            player.SendMessageUpwards("enemyDefeated", 1000);       //Exp Granted
            player.SendMessageUpwards("addGold", 5000);             //Gold Given
            deathTimer = deathDelay;
            dtime_start = true;
            if (PlayerPrefs.GetInt("storyProgression") < 3)
            {
                PlayerPrefs.SetInt("storyProgression", 3);
            }
        }
