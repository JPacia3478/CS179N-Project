...
public class EnemySpaghet : MonoBehaviour
{
    //attributes
    public int Level;
    public int Exp;
    ...
    void Update()
    {
		...
		if(HP <= 0 && !defeat)
		{
            ...
            player.SendMessageUpwards("enemyDefeated", 1000);           //Exp Given
            player.SendMessageUpwards("addGold", 5000);                 //Gold Given
            deathTimer = deathDelay;
            dtime_start = true;
            if (PlayerPrefs.GetInt("storyProgression") < 4)
            {
                PlayerPrefs.SetInt("storyProgression", 4);
            }
        }
